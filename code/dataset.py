...
class Generator:
    ...
    def make_dataset(self, digit_type):
        ...
        for resolution in Generator.__RESOLUTIONS:
            ...
            self.__parse_digits(game_window, digit_type, resolution)
    ...
    def __parse_digits(self, game_window, digit_type, resolution):
        ...
        path = self.__save_path+'/'+digit_type+"_digits"
        ...
        if digit_type == 'counter':
            ...
            for cell in parser.parse_cells(screenshot, Cell.ORANGE_OLD):
                ...
                cv2.imwrite(path+'/{0}_{1}.png'.format(remaining, resolution), remaining_img)

                game_window.click_cell(cell, 'left')
                remaining -= 1
                screenshot = game_window.screenshot()

            cv2.imwrite(path+'/{0}_{1}.png'.format(0, resolution), mistakes_img)

        elif digit_type == 'black':
            images = parser.parse_cells(screenshot, Cell.BLACK, training=True)
            digits = ['{1}', '{2}', '{3}', '{4}', '{5}', '{6}',
                      '1', '2', '3', '4', '5', '6', 'question-mark', 
                      '-2-', '-3-', '-4-', '{0}', '0']

            for image, digit in list(zip(images, digits)):
                cv2.imwrite(path+'/{0}_{1}.png'.format(digit, resolution), image)

        elif digit_type == 'blue':
            images = parser.parse_cells(screenshot, Cell.BLUE, training=True)

            for image, digit in list(zip(images, range(0, 19))):
                cv2.imwrite(path+'/{0}_{1}.png'.format(digit, resolution), image)
